  ...
  return (
    <nav
      className={`fixed w-full z-50 transition-all duration-500 ${
        scrolled ? "bg-brownCoffee shadow-2xl py-3" : "bg-transparent py-6"
      }`}
    >
      {/* Top bar */}
      {!scrolled && (
          ...
          <div className="container mx-auto px-6 flex justify-between items-center text-[10px] tracking-widest font-medium text-white/80">
            <div className="flex items-center space-x-6">
              <span className="flex items-center">
                <i className="fa-solid fa-phone mr-2 text-uocGold"></i> +91
                88280 06599
              </span>
              <span className="flex items-center">
                <i className="fa-solid fa-envelope mr-2 text-uocGold"></i>{" "}
                holidays@travkings.com
              </span>
            </div>
      ...
      <div className="container mx-auto px-6 flex justify-between items-center">
        <Link href="/" className="flex items-center group">
          ...
          <Image
            ...
            height={48}
            priority
            className={`lg:hidden w-auto transition-all duration-500 ${
              scrolled ? "h-10" : "h-12"
            }`}
          />
          <Image
            ...
            height={64}
            priority
            className={`hidden lg:block w-auto transition-all duration-500 ${
              scrolled ? "h-12" : "h-16"
            }`}
          />
        </Link>
        ...
        <div className="hidden lg:flex items-center space-x-10">
          {["Home", "About", "Services", "Packages", "Blogs", "Contact"].map(
            (item) => (
              <Link
                key={item}
          ...
          <Link
            href="/contact"
            className="bg-brandy hover:bg-brownCoffee border border-brandy hover:border-uocGold text-white px-8 py-3 rounded-none text-xs font-medium uppercase tracking-widest transition-all duration-300 shadow-lg"
          >
            Book Now
          </Link>
        </div>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 bg-brownCoffee z-60 transform transition-transform duration-500 lg:hidden ${
